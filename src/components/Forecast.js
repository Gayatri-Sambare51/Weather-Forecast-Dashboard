...
import '../CSS/Forecast.css'; // Import CSS file
...
function Forecast({ city, unit }) {
    ...
    useEffect(() => {
        const fetchForecast = async () => {
            try {
                ...
                console.error('Error fetching forecast:', error); // Set error message
                setError('Error fetching forecast data. Please try again.'); // Set error message
            }
        };
    ...
    const getFiveDayForecast = (forecastData) => {
        ...
        // Push the last day's average temperature
        if (currentDate) {
            const avgTemp = tempSum / tempCount;
            fiveDayForecast.push({
                date: currentDate,
